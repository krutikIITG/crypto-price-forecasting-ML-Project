...
from statsmodels.tsa.ar_model import AutoReg  # Updated import
warnings.filterwarnings("ignore")

print("Starting ARMA model with Binance API...")
...
# AutoReg Model (replaces AR)
print("Training AutoReg model...")
try:
    model = AutoReg(train_y, lags=5)  # Updated: AR replaced with AutoReg
    arma_model = model.fit()
    
    print("Generating predictions...")
    predictions = arma_model.predict(start=len(train_y), end=len(train_y) + len(test_y) - 1)
    ...
    plt.legend()
    plt.title('AutoReg (ARMA) Bitcoin Price Prediction (365 Days)')
    plt.show()
    ...
    print("RMSE:", error)
    print("AutoReg (ARMA) model completed successfully!")

except Exception as e:
    print(f"Error in AutoReg: {e}")
